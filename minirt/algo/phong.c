...
/*   Updated: 2025/03/11 12:50:12 by aloubry          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_ray_collision(t_ray *ray, t_list *shapes, float max_distance)
{
	float	distance;
...
static t_color	compute_final_color(t_scene *scene, t_ray_hit_info *hit_info,
...
t_color	apply_phong(t_scene *scene, t_ray_hit_info *hit_info)
{
	...
	if (hit_info->shape->texture && uvs.x != -1 && uvs.y != -1)
		point_color = color_add(point_color,
				sample_image(uvs, hit_info->shape->texture));
